...
from src.schwab import cancel_order, fetch_account_data, fetch_option_chain, fetch_option_expiration_chain, fetch_orders_for_account, fetch_quote, fetch_quotes, place_order
...
async def get_dividend_yield(ticker):
    ...
    """
    div_data = await fetch_quote(ticker)
...
async def fetch_streamer_quotes_and_calculate_deltas(ticker, streamers_tickers, expiration_time, options, total_shares, r, q):
    ...
    stock_quote_data = await fetch_quote(ticker)
    ...
    options_quote_data = await fetch_quotes(streamers_tickers)
...
async def adjust_delta_imbalance(ticker, delta_imbalance, config, is_closing_position=False):
    ...
    if delta_imbalance > 0:
        ...
        if not config["DRY_RUN"]:
            ...
            await place_order(config["SCHWAB_ACCOUNT_HASH"], order)
    else:
...
async def get_option_chain_data(ticker, option_date, option_type):
    ...
    chain = await fetch_option_chain(ticker, option_date, option_type)
    if not chain:
        return quote_data, S
